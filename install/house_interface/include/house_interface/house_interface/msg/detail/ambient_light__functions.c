// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from house_interface:msg/AmbientLight.idl
// generated code does not contain a copyright notice
#include "house_interface/msg/detail/ambient_light__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


bool
house_interface__msg__AmbientLight__init(house_interface__msg__AmbientLight * msg)
{
  if (!msg) {
    return false;
  }
  // structure_needs_at_least_one_member
  return true;
}

void
house_interface__msg__AmbientLight__fini(house_interface__msg__AmbientLight * msg)
{
  if (!msg) {
    return;
  }
  // structure_needs_at_least_one_member
}

bool
house_interface__msg__AmbientLight__are_equal(const house_interface__msg__AmbientLight * lhs, const house_interface__msg__AmbientLight * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // structure_needs_at_least_one_member
  if (lhs->structure_needs_at_least_one_member != rhs->structure_needs_at_least_one_member) {
    return false;
  }
  return true;
}

bool
house_interface__msg__AmbientLight__copy(
  const house_interface__msg__AmbientLight * input,
  house_interface__msg__AmbientLight * output)
{
  if (!input || !output) {
    return false;
  }
  // structure_needs_at_least_one_member
  output->structure_needs_at_least_one_member = input->structure_needs_at_least_one_member;
  return true;
}

house_interface__msg__AmbientLight *
house_interface__msg__AmbientLight__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  house_interface__msg__AmbientLight * msg = (house_interface__msg__AmbientLight *)allocator.allocate(sizeof(house_interface__msg__AmbientLight), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(house_interface__msg__AmbientLight));
  bool success = house_interface__msg__AmbientLight__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
house_interface__msg__AmbientLight__destroy(house_interface__msg__AmbientLight * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    house_interface__msg__AmbientLight__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
house_interface__msg__AmbientLight__Sequence__init(house_interface__msg__AmbientLight__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  house_interface__msg__AmbientLight * data = NULL;

  if (size) {
    data = (house_interface__msg__AmbientLight *)allocator.zero_allocate(size, sizeof(house_interface__msg__AmbientLight), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = house_interface__msg__AmbientLight__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        house_interface__msg__AmbientLight__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
house_interface__msg__AmbientLight__Sequence__fini(house_interface__msg__AmbientLight__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      house_interface__msg__AmbientLight__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

house_interface__msg__AmbientLight__Sequence *
house_interface__msg__AmbientLight__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  house_interface__msg__AmbientLight__Sequence * array = (house_interface__msg__AmbientLight__Sequence *)allocator.allocate(sizeof(house_interface__msg__AmbientLight__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = house_interface__msg__AmbientLight__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
house_interface__msg__AmbientLight__Sequence__destroy(house_interface__msg__AmbientLight__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    house_interface__msg__AmbientLight__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
house_interface__msg__AmbientLight__Sequence__are_equal(const house_interface__msg__AmbientLight__Sequence * lhs, const house_interface__msg__AmbientLight__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!house_interface__msg__AmbientLight__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
house_interface__msg__AmbientLight__Sequence__copy(
  const house_interface__msg__AmbientLight__Sequence * input,
  house_interface__msg__AmbientLight__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(house_interface__msg__AmbientLight);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    house_interface__msg__AmbientLight * data =
      (house_interface__msg__AmbientLight *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!house_interface__msg__AmbientLight__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          house_interface__msg__AmbientLight__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!house_interface__msg__AmbientLight__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
